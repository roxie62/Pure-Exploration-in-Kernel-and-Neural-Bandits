...
def calculate_effective_dimension(X, eps, epsilon_d, theta_norm = 0.5):
    ...
    print('original dimension is:', d, '\n')
    print('d_r is', d_r, '!!!!!!', '\n')
    return d_r, X_r, gamma_tilde_d


class linear_elim(object):
    def __init__(self, X, reward_func, factor, delta, epsilon_d = 0.1, theta_norm = 1, Z=None):
    ...
    def algorithm(self, seed, binary=False):
        ...
        self.phase_index = 1
        eps = 1/self.factor
        self.d_r, self.X_sample, self.gamma_tilde_d = calculate_effective_dimension(self.X_sample, eps, \
                                                        self.epsilon_d, theta_norm = self.theta_norm)
        # self.gamma_tilde_d = 0
        ...
        while len(self.active_arms) > 1:
            ...
            if len(list(set(self.opt_arm) & set(self.active_arms))) == len(self.active_arms):
                print('stopped, phase number {}'.format(self.phase_index))
                break
            else:
            ...
            self.build_Y()
            design, rho = self.optimal_allocation_fixed()
            ...
            print('num_samples', num_samples, 'rho', rho, 'multiplier', ((2**(-self.phase_index) - self.epsilon_k)** (-2)) *2)
        ...
        logging.critical('linear_elim Sample complexity %s' % str(self.N))
    ...
    def optimal_allocation_fixed(self):
        ...
        for count in range(1, max_iter):
            ...
            try:
                ...
                U,D,V = np.linalg.svd(A_inv + 1e-5)
            Ainvhalf = U@np.diag(np.sqrt(D))@V
