...
def calculate_effective_dimension(X, eps, epsilon_d, theta_norm = 0.5):
    ...
    # calculate effective dimension d_r
    d_r = np.min(rr[filter])
    gamma_tilde_d = sigma_rr[d_r - 1]
    # Output the empirical Phi(x) with shape (K x d_r)
...
def kernel_mapping(X, gamma, gaussian = True):
    ...
    if gaussian:
        ...
        # print('empirical variance:', 1 / (2 * np.var(sq_dist)))
        mat_sq_dists = squareform(sq_dist)
        K = np.exp(-gamma * mat_sq_dists)
    return K


class kernel_elim(object):
    ...
    def algorithm(self, seed, binary= False):
        ...
        self.d_r, self.X_sample, self.gamma_tilde_d = calculate_effective_dimension(self.X_sample, eps, self.epsilon_d, theta_norm = self.theta_norm)
        ...
        logging.critical('kernel_elim Sample complexity %s' % str(self.N))
    ...
    def drop_arms(self):
        ...
        for arm_idx in active_arms:
            ...
            for arm_idx_prime in active_arms:
                ...
                y = arm_prime - arm
                # if self.epsilon_k + np.sqrt(2*np.abs(y.T@self.A_inv@y)*np.log(2*self.K**2/self.delta_t)) <= y.T@self.theta_hat:
